...
Let's start with creating the layout that has two `Buttons` increment, decrement, and, a `TextView` to show the current value of the counter.\
<a href="https://github.com/eliekarouz/FeedbackTree/blob/master/app/src/main/res/layout/counter.xml" target="_blank">Get the code</a>

The `CounterFlow` produces a `CounterScreen` for each `State`. What we need to complete the puzzle is some code that will create the corresponding `Layout`, and, update its element when the state updates. 
...
when (event) {
    ...
    Event.Decrement -> state.copy(
        counter = max(0, state.counter - 1) // prevents the state going negative
    ).advance()
}
```
