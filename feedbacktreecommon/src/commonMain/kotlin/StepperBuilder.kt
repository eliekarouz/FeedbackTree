 ...
 * @param EventT
 */
class StepperBuilder<StateT : Any, EventT : Any, OutputT : Any> private constructor() {
    ...
    inner class SubStateSteppers<S : StateT, EventT : Any, OutputT : Any>(val transitions: MutableMap<Matcher<EventT, EventT>, S.(EventT) -> Step<StateT, OutputT>> = mutableMapOf()) {
        inline fun <reified E : EventT> on(noinline transition: S.(E) -> Step<StateT, OutputT>) {
            val matcher = Matcher<EventT, E>(E::class)
    ...
    inline fun <reified S : StateT> state(build: SubStateSteppers<S, EventT, OutputT>.() -> Unit) {
        ...
        val matcher = Matcher<StateT, S>(S::class)
    ...
    companion object {
        fun <StateT : Any, EventT : Any, OutputT : Any> create(build: StepperBuilder<StateT, EventT, OutputT>.() -> Unit): (StateT, EventT) -> Step<StateT, OutputT> {
            ...
            return { state, event ->
                ...
                transition?.transition(state, event) ?: run {
                    ...
                    Step.Advance<StateT, OutputT>(newState = state) // stay on the same state
                }
            }
        }
    }
